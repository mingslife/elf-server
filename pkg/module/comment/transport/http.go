...
import (
	...
	"elf-server/pkg/module/comment/endpoint"
)

type CommentHttp struct {
	Router   *bone.Router              `inject:"application.router"`
	Endpoint *endpoint.CommentEndpoint `inject:""`
	Decoder  *CommentDecoder           `inject:""`
}

func (t *CommentHttp) Register() error {
	o, j := http.MethodOptions, httptransport.EncodeJSONResponse
	s := t.Router.PathPrefix("/api/v1/comments").Subrouter()
...
var _ bone.Transport = (*CommentHttp)(nil)
