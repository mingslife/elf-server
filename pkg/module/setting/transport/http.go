...
import (
	...
	"elf-server/pkg/module/setting/endpoint"
)

type SettingHttp struct {
	Router   *bone.Router              `inject:"application.router"`
	Endpoint *endpoint.SettingEndpoint `inject:""`
	Decoder  *SettingDecoder           `inject:""`
}

func (t *SettingHttp) Register() error {
	o, j := http.MethodOptions, httptransport.EncodeJSONResponse
	s := t.Router.PathPrefix("/api/v1/settings").Subrouter()
...
var _ bone.Transport = (*SettingHttp)(nil)
