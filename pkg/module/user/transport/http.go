...
import (
	...
	"elf-server/pkg/module/user/endpoint"
)

type UserHttp struct {
	Router   *bone.Router           `inject:"application.router"`
	Endpoint *endpoint.UserEndpoint `inject:""`
	Decoder  *UserDecoder           `inject:""`
}

func (t *UserHttp) Register() error {
	o, j := http.MethodOptions, httptransport.EncodeJSONResponse
	s := t.Router.PathPrefix("/api/v1/users").Subrouter()
...
var _ bone.Transport = (*UserHttp)(nil)
